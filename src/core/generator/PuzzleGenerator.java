...
public class PuzzleGenerator {
    ...
    public Variable[][] variables = new Variable[9][9];
    ...
    private void initialize() {
        ...
        /** Converting Variable array to integer array for ease of operation */
    ...
    private void performColumnSwap() {
        for (int row = 0; row < variables.length; row++) {
            for (int col = 0; col < variables[0].length - 3; col += 3) {
                Variable temp = variables[row][col];
        ...
        try {
            ...
            while (currentNumOfGivens != givens) {
                ...
                if (puzzleGrid[row][col] > 0 && puzzleGrid[col][row] > 0) {
                    ...
                    int numSolutions = solver.label.getSolutionListener()
                            .solutionsNo();
